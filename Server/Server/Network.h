...
public:
	...
	MSG_TYPE Help(const SOCKET& sock, const vector<string>& vecMsg, const uint& roomNumber); // 명령어 도움말 전송
	MSG_TYPE ShowRoomAll(const SOCKET& sock, const vector<string>& vecMsg, const uint& roomNumber); // 대화방 목록 출력
	MSG_TYPE ShowRoom(const SOCKET& sock, const vector<string>& vecMsg, const uint& roomNumber); // 특정 대화방 정보 출력
	...
	MSG_TYPE DestoryRoom(const SOCKET& sock, const vector<string>& vecMsg, const uint& roomNumber); // 대화방 폭파
	MSG_TYPE ShowUserAll(const SOCKET& sock, const vector<string>& vecMsg, const uint& roomNumber); // 유저 목록 출력
	MSG_TYPE ShowUser(const SOCKET& sock, const vector<string>& vecMsg, const uint& roomNumber); // 특정 유저 정보 출력
	MSG_TYPE SendMsg(const SOCKET& sock, const vector<string>& vecMsg, const uint& roomNumber); // 쪽지보내기(귓속말)
	MSG_TYPE Close(const SOCKET& sock, const vector<string>& vecMsg, const uint& roomNumber); // 접속 종료
};
